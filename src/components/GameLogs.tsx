...
import { PHASE_NAMES } from '../shared/types';

type LogTab = 'public' | 'private' | 'god';

export function GameLogs({ game }: { game: GameView }) {
  ...
  const logs = useMemo(() => {
    ...
    return source
      .filter((log) => {
        ...
        const searchable = `第 ${log.day} 天 ${PHASE_NAMES[log.phase]} ${log.message}`;
        return searchable.toLocaleLowerCase('zh-CN').includes(needle);
      })
      .slice()
      .reverse();
    ...
    <section className="logs" id="game-logs">
      <div className="tabs" role="tablist" aria-label="日志类型">
        {(['public', 'private', 'god'] as const).map((item) => (
          ...
          >
            {item === 'public' ? '公共' : item === 'private' ? '私人' : '上帝'}
            <i>{counts[item]}</i>
          </button>
        ))}
      </div>
      ...
      <div className="log-search">
        ...
        />
        {query && (
          <button onClick={() => setQuery('')} aria-label="清除日志搜索">
            ×
          </button>
        )}
      </div>
