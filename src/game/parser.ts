...
export function parseReply(raw:string): ParsedAction[] {
  const text = decode(raw).replace(/<[^>]*>/g, ' ').replace(/\*\*/g, '').replace(/\s+/g, ' ');
  const results: ParsedAction[] = [];
  const patterns: Array<[RegExp,(m:RegExpMatchArray)=>ParsedAction]> = [
  ...
  for (const [pattern, create] of patterns) for (const match of text.matchAll(pattern)) results.push(create(match));
  return results;
}
